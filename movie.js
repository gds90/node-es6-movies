...
// Funzione precedente fatta col destructuring durante correzione in aula, evitando di specificare media. per ogni creazione di istanza
const mediaArrayMapped1 = mediaArray.map(({ title, genre, year, rating, type, seasons }) => {
    if (type === "movie") {
...
// metodo con reduce:
// const genres = mediaArray.reduce( (accum, media) => !genres.includes(item.genre) ? [...accum, media.genre] : accum,  []);
